...
public:
	...
	int GetNVertice() { return m_nVertices;	}
	XMFLOAT3 GetPosition(UINT idx) { return(m_pxmf3Positions[idx]); }
};
...
public:
	CPointMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, XMFLOAT3* center, XMFLOAT2* size);
	CPointMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, XMFLOAT3* center, XMFLOAT3* size);
	virtual ~CPointMesh();
