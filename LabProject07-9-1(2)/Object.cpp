...
{
	pd3dCommandList->SetGraphicsRootConstantBufferView(3, m_pd3dcbGameObject->GetGPUVirtualAddress());
...
{
	UINT ncbBytes = ((sizeof(VS_VB_INSTANCE) + 255) & ~255);
...
void CGameObject::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	// 1) 머티리얼 색상 채우기
	if (m_nMaterials > 0 && m_ppMaterials[0] && m_ppMaterials[0]->m_pMaterialColors)
	{
		m_pcbMappedGameObject->material.m_cAmbient = m_ppMaterials[0]->m_pMaterialColors->m_xmf4Ambient;
		m_pcbMappedGameObject->material.m_cDiffuse = m_ppMaterials[0]->m_pMaterialColors->m_xmf4Diffuse;
		m_pcbMappedGameObject->material.m_cSpecular = m_ppMaterials[0]->m_pMaterialColors->m_xmf4Specular;
		m_pcbMappedGameObject->material.m_cEmissive = m_ppMaterials[0]->m_pMaterialColors->m_xmf4Emissive;
	}

	// 2) 월드 행렬 채우기 (Transpose 해서 넣음)
	XMStoreFloat4x4(
