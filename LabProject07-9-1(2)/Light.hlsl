...
float4 Inst_DirectionalLight(int nIndex, float3 vNormal, float3 vToCamera, MATERIAL material)
{
    float3 vToLight = -gLights[nIndex].m_vDirection;
    float fDiffuseFactor = dot(vToLight, vNormal);
    float fSpecularFactor = 0.0f;
...
float4 Inst_PointLight(int nIndex, float3 vPosition, float3 vNormal, float3 vToCamera, MATERIAL material)
{
    float3 vToLight = gLights[nIndex].m_vPosition - vPosition;
    float fDistance = length(vToLight);
    if (fDistance <= gLights[nIndex].m_fRange)
...
float4 Inst_SpotLight(int nIndex, float3 vPosition, float3 vNormal, float3 vToCamera, MATERIAL material)
{
    float3 vToLight = gLights[nIndex].m_vPosition - vPosition;
    float fDistance = length(vToLight);
    if (fDistance <= gLights[nIndex].m_fRange)
...
float4 Inst_ComputeLighting(float3 vPosition, float3 vNormal, MATERIAL material)
{
    float3 vCameraPosition = float3(gvCameraPosition.x, gvCameraPosition.y, gvCameraPosition.z);
    float3 vToCamera = normalize(vCameraPosition - vPosition);
    ...
    for (int i = 0; i < gnLights; i++)
    {
        if (gLights[i].m_bEnable)
        {
            if (gLights[i].m_nType == DIRECTIONAL_LIGHT)
            {
                cColor += Inst_DirectionalLight(i, vNormal, vToCamera, material);
            }
            else if (gLights[i].m_nType == POINT_LIGHT)
            {
                cColor += Inst_PointLight(i, vPosition, vNormal, vToCamera, material);
            }
            else if (gLights[i].m_nType == SPOT_LIGHT)
            {
                cColor += Inst_SpotLight(i, vPosition, vNormal, vToCamera, material);
            }
        }
    }
