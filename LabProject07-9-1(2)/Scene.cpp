...
{
	if (!pObject) {
		OutputDebugString(L"[AssignCbvRecursive] pObject == nullptr\n");
		return;
	}

	OutputDebugString(L"[AssignCbvRecursive] Assigning CBV...\n");

	// 자기 자신의 CBV 할당
	pObject->SetCbvDescriptorHandles(d3dCPUHandle, d3dGPUHandle);
	pObject->CreateShaderVariables(pd3dDevice, pd3dCommandList);

	if (pObject->m_d3dCbvGPUDescriptorHandle.ptr == 0)
		OutputDebugString(L"GPU handle still 0 after CreateShaderVariables!\n");
...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Instance
	int xObjects = 10, yObjects = 10, zObjects = 10, k = 0;
	m_nInstanceGameObjects = (xObjects * 2 + 1) * (yObjects * 2 + 1) * (zObjects * 2 + 1);
	m_ppInstanceGameObjects = new CGameObject * [m_nInstanceGameObjects];

	float fxPitch = 12.0f * 2.5f;
	float fyPitch = 12.0f * 2.5f;
	float fzPitch = 12.0f * 2.5f;

	CApacheObject* pInstanceApacheObject = NULL;

	for (int x = -xObjects; x <= xObjects; x++) {
		for (int y = -yObjects; y <= yObjects; y++) {
			for (int z = -zObjects; z <= zObjects; z++) {
				pInstanceApacheObject = new CApacheObject();
				pInstanceApacheObject->SetPosition(fxPitch*x, fyPitch * y, fzPitch * z);
				pInstanceApacheObject->SetScale(1.5f, 1.5f, 1.5f);
				pInstanceApacheObject->Rotate(0.0f, 90.0f, 0.0f);

				m_ppInstanceGameObjects[k++] = pInstanceApacheObject; 
			}
		}
	}

	CGameObject* pInstanceApacheModel = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, "Model/Apache.bin");
	m_ppInstanceGameObjects[0]->SetChild(pInstanceApacheModel);
	m_ppInstanceGameObjects[0]->OnInitialize();
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	AssignCbvRecursive(m_ppInstanceGameObjects[0], pd3dDevice, pd3dCommandList,
		d3dCPUHandle, d3dGPUHandle, nIncrementSize);
...
void CScene::ReleaseObjects()
{
	...
	if (m_ppInstanceGameObjects)
	{
		for (int i = 0; i < m_nInstanceGameObjects; i++) if (m_ppInstanceGameObjects[i]) m_ppInstanceGameObjects[i]->Release();
		delete[] m_ppInstanceGameObjects;
	}
...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dRootParameters[3].Descriptor.ShaderRegister = 0; //Instance
...
void CScene::ReleaseUploadBuffers()
{
	...
	for (int i = 0; i < m_nInstanceGameObjects; i++) m_ppInstanceGameObjects[i]->ReleaseUploadBuffers();
}
...
void CScene::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	if (m_ppInstanceGameObjects[0])
	{
		m_ppInstanceGameObjects[0]->Animate(m_fElapsedTime, NULL);
		m_ppInstanceGameObjects[0]->UpdateTransform(NULL);
		m_ppInstanceGameObjects[0]->Render(pd3dCommandList, pCamera, m_nInstanceGameObjects);
	}
}
