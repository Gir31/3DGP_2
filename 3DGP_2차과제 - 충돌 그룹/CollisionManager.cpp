...
void CollisionManager::CheckSphereCollision(ID3D12GraphicsCommandList* pd3dCommandList, std::vector<CGameObject*>& gameObjects)
{
	...
	for (size_t i = 0; i < count - 1; ++i)
	{
		...
		for (size_t j = i + 1; j < count; ++j)
		{
			...
			if (check & NONE_CHECK) continue;
			if (check & ALL_STATIC) continue;
...
bool CollisionManager::CheckCollisionGroup(const SRV_BOUNDINGBOX_INFO& A, const int CollisionGroupA, const XMFLOAT4X4& worldA, 
	const SRV_BOUNDINGBOX_INFO& B, const int CollisionGroupB, const XMFLOAT4X4& worldB)
{
	int CollisionGroup = CollisionGroupA | CollisionGroupB;
