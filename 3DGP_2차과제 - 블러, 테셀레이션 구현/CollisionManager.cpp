...
{
	m_pd3dSphereDefault = NULL;
	m_pd3dBoundingBoxDefault = NULL;
...
{
	if (m_pd3dSphereDefault)
	{
		m_pd3dSphereDefault->Unmap(0, NULL);
		m_pd3dSphereDefault->Release();
	}

	if (m_pd3dSphereUpload)
	{
		m_pd3dSphereUpload->Unmap(0, NULL);
		m_pd3dSphereUpload->Release();
	}

	if (m_pd3dBoundingBoxDefault)
	{
		m_pd3dBoundingBoxDefault->Unmap(0, NULL);
		m_pd3dBoundingBoxDefault->Release();
	}

	if (m_pd3dBoundingBoxUpload)
	{
		m_pd3dBoundingBoxUpload->Unmap(0, NULL);
		m_pd3dBoundingBoxUpload->Release();
	}
...
{
	if (!m_pd3dSphereDefault) return;
	if (m_vSphereInfo.empty()) return;

	const UINT totalSize = (UINT)(sizeof(SRV_SPHERE_INFO) * m_vSphereInfo.size());

	ID3D12Resource* pDstDefault = m_pd3dSphereDefault;
	ID3D12Resource* pSrcUpload = m_pd3dSphereUpload;
...
{
	if (!m_pd3dBoundingBoxDefault) return;
	if (m_vSphereInfo.empty()) return;

	const UINT totalSize = (UINT)(sizeof(SRV_BOUNDINGBOX_INFO) * m_vBoundingBoxInfo.size());


	ID3D12Resource* pDstDefault = m_pd3dBoundingBoxDefault;
	ID3D12Resource* pSrcUpload = m_pd3dBoundingBoxUpload;
		...
		for (const auto& boundBoxIndexA : objA->m_vBoundingBoxIndices)
		{
			...
			for (const auto& boundBoxIndexB : objB->m_vBoundingBoxIndices)
			{
				...
				if (CheckCollisionGroup(boxA, worldA, collisionGroupA, boxB, worldB, collisionGroupB))
				{
					...
					if (collisionGroupA & CG_ENEMY)
					{
						objA->die = true;
					}
					else if (collisionGroupB & CG_ENEMY)
					{
						objA->die = true;
					}
