...
void CTexturedRectMesh::Render(ID3D12GraphicsCommandList *pd3dCommandList, int nSubSet, UINT nInstances)
{
	pd3dCommandList->IASetPrimitiveTopology(m_d3dPrimitiveTopology);
...
void CStandardMesh::Render(ID3D12GraphicsCommandList *pd3dCommandList, int nSubSet, UINT nInstances)
{
	pd3dCommandList->IASetPrimitiveTopology(m_d3dPrimitiveTopology);
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	for (int z = 0; z < nLength - 1; z++)
	{
		for (int x = 0; x < nWidth - 1; x++)
		{
			...
			UINT i2 = (x + 1) + ((z + 1) * nWidth);
			UINT i3 = x + ((z + 1) * nWidth);
...
void CHeightMapGridMesh::Render(ID3D12GraphicsCommandList* pd3dCommandList, int nSubSet, UINT nInstances)
{
	pd3dCommandList->IASetPrimitiveTopology(m_d3dPrimitiveTopology);
...
// 
// 평면
CPointMesh::CPointMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, XMFLOAT3* center, XMFLOAT2* size) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 1;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_POINTLIST;

	m_pxmf3Positions = new XMFLOAT3();
	m_pxmf3Positions[0] = *center;
...
// 큐브
CPointMesh::CPointMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, XMFLOAT3* center, XMFLOAT3* size) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 1;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_POINTLIST;
	...
	m_pd3dSizeBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList,
		...
		&m_pd3dSizeUploadBuffer);
	m_d3dSizeBufferView.BufferLocation = m_pd3dSizeBuffer->GetGPUVirtualAddress();
	m_d3dSizeBufferView.StrideInBytes = sizeof(XMFLOAT3);
	m_d3dSizeBufferView.SizeInBytes = sizeof(XMFLOAT3);
}
...
void CPointMesh::Render(ID3D12GraphicsCommandList* pd3dCommandList, int nSubSet, UINT nInstances)
{
	...
	pd3dCommandList->DrawInstanced(1, nInstances, 0, 0);
}