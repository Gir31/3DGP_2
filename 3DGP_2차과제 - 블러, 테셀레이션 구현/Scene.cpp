...
void CScene::ComputeMirrorReflectionCamera(CCamera* pMainCamera, CCamera* pReflectionCamera)
{

	// 1) 거울 평면 정의
	XMFLOAT3 mirrorCenter(920.f, 1000.f, 1270.f);
	XMFLOAT3 mirrorNormal(0.f, 0.f, 1.f); // z=1270 평면, 노멀 +Z

	XMVECTOR N = XMVector3Normalize(XMLoadFloat3(&mirrorNormal));
	XMVECTOR M = XMLoadFloat3(&mirrorCenter);

	float d = -XMVectorGetX(XMVector3Dot(N, M));
	XMVECTOR plane = XMVectorSet(N.m128_f32[0], N.m128_f32[1], N.m128_f32[2], d);

	XMMATRIX R = XMMatrixReflect(plane);

	// 2) 메인 카메라 정보
	XMVECTOR eye = XMLoadFloat3(&pMainCamera->GetPosition());
	XMVECTOR look = XMLoadFloat3(&pMainCamera->GetLookVector());
	//XMVECTOR up   = XMLoadFloat3(&pMainCamera->GetUpVector()); // 안 씀

	XMVECTOR at = eye + look;

	// 3) 위치와 시점은 반사
	XMVECTOR eyeR = XMVector3TransformCoord(eye, R);
	XMVECTOR atR = XMVector3TransformCoord(at, R);

	// 4) Up은 "롤을 무시하고" 월드 Up을 사용
	XMVECTOR worldUp = XMVectorSet(0.f, 1.f, 0.f, 0.f);

	// 5) look/right/up 재계산
	XMVECTOR lookR = XMVector3Normalize(atR - eyeR);
	XMVECTOR rightR = XMVector3Normalize(XMVector3Cross(worldUp, lookR));
	XMVECTOR upR = XMVector3Cross(lookR, rightR);

	XMFLOAT3 fEyeR, fAtR, fLookR, fUpR, fRightR;
	XMStoreFloat3(&fEyeR, eyeR);
	XMStoreFloat3(&fAtR, atR);
	XMStoreFloat3(&fLookR, lookR);
	XMStoreFloat3(&fUpR, upR);
	XMStoreFloat3(&fRightR, rightR);

	// 6) 카메라에 세팅
	pReflectionCamera->SetPosition(fEyeR);
	pReflectionCamera->SetLookAtPosition(fAtR);

	pReflectionCamera->m_xmf3Look = fLookR;
	pReflectionCamera->m_xmf3Up = fUpR;
	pReflectionCamera->m_xmf3Right = fRightR;

	XMMATRIX viewR = XMMatrixLookAtLH(eyeR, atR, upR);
	XMStoreFloat4x4(&pReflectionCamera->m_xmf4x4View, viewR);
}
...
void CScene::AddBillboardInfo()
{
	if (m_nShaders > BILLBOARD_SHADER)
	{
		...
		int nTerrainWidth = int(pTerrain->GetWidth() / 100);
		int nTerrainLength = int(pTerrain->GetLength() / 100);
		...
		for (int x = 0; x < nTerrainWidth; ++x)
		{
			for (int z = 0; z < nTerrainLength; ++z)
			{
				...
				billboard.xmf2Size = XMFLOAT2(100.f, 80.f);
