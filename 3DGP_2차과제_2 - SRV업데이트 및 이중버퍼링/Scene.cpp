...
void CScene::CreateShaderResourceView(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		if (m_pd3dcbGameObjects[i]) { m_pd3dcbGameObjects[i]->Release(); m_pd3dcbGameObjects[i] = nullptr; }
		if (m_pd3dUploadBuffer[i]) { m_pd3dUploadBuffer[i]->Release();  m_pd3dUploadBuffer[i] = nullptr; }

		m_pd3dcbGameObjects[i] = ::CreateBufferResource(
		...
		pd3dCommandList->CopyResource(m_pd3dcbGameObjects[i], m_pd3dUploadBuffer[i]);

		D3D12_RESOURCE_BARRIER barrier = {};
		barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		barrier.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
		barrier.Transition.pResource = m_pd3dcbGameObjects[i];
	...
	for (int i = 0; i < 2; ++i)
	{
		pd3dDevice->CreateShaderResourceView(m_pd3dcbGameObjects[i], &srvDesc, m_pDescriptorHeap->m_d3dCPUDescriptorHandle);
...
{
	if (!m_pd3dcbGameObjects[0] || !m_pd3dcbGameObjects[1]) return;
	...
	ID3D12Resource* pDstDefault = m_pd3dcbGameObjects[frame];
