...
void CStandardMesh::LoadMeshFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, FILE *pInFile)
{
	...
	for ( ; ; )
	{
		...
		if (!strcmp(pstrToken, "<Bounds>:"))
		{
			...
			char debugMsg[256];
			sprintf_s(debugMsg,
				"[Mesh Debug] <Bounds> Center(%.3f, %.3f, %.3f) | Extents(%.3f, %.3f, %.3f)\n",
				m_xmf3AABBCenter.x, m_xmf3AABBCenter.y, m_xmf3AABBCenter.z,
				m_xmf3AABBExtents.x, m_xmf3AABBExtents.y, m_xmf3AABBExtents.z);
