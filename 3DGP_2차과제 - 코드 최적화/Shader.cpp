    ...
    for (int h = 0; h < nFirstPassColumnSize; h++)
    {
        for (int i = 0; i < floor(float(m_nGameObjects) / float(nColumnSize)); i++)
        {
			if (nObjects % 2)
			{
				...
				m_ppGameObject[nObjects]->SetChild(pSuperCobraModel->Clone());
			}
			else
			{
				m_ppGameObject[nObjects] = new CGunshipObject(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
				m_ppGameObject[nObjects]->SetChild(pGunshipModel->Clone());
			}
...
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSSphere", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CSphereShader::CreateGeometryShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "GSSphere", "gs_5_1", &m_pd3dGeometryShaderBlob));
}

D3D12_SHADER_BYTECODE CSphereShader::CreatePixelShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSSphere", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "GSBoundingBox", "gs_5_1", &m_pd3dGeometryShaderBlob));
}
...
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "GSUI", "gs_5_1", &m_pd3dGeometryShaderBlob));
}
...
D3D12_DEPTH_STENCIL_DESC CMirrorShader::CreateDepthStencilState()
{
	...
	d.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	d.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	d.FrontFace.StencilPassOp = D3D12_STENCIL_OP_REPLACE;
	d.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;
...
{
	D3D12_BLEND_DESC desc = {};
	desc.RenderTarget[0].RenderTargetWriteMask = 0; // 색 안그리기
	return desc;
}
...
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSMirrorScene", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CMirrorShader::CreatePixelShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSMirrorScene", "ps_5_1", &m_pd3dPixelShaderBlob));
}
