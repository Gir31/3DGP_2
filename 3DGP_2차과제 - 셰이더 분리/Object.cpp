...
void CGameObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, UINT nInstances)
{
	...
	pd3dCommandList->SetGraphicsRoot32BitConstants(13, 1, &SRVIndex, 0);
	...
	if (m_nMaterials > 1)
	{
		for (int i = 0; i < m_nMaterials; i++)
		{
			...
			if (m_nMeshes == 1)
			{
				if (m_ppMeshes[0]) m_ppMeshes[0]->Render(pd3dCommandList, i, nInstances);
			}
		}
	}
	else
	{
		...
		if (m_ppMeshes)
		{
			for (int i = 0; i < m_nMeshes; i++)
			{
				if (m_ppMeshes[i]) m_ppMeshes[i]->Render(pd3dCommandList, 0, nInstances);
			}
		}
	}
...
{
	CGameObject* pClone = new CGameObject(m_nMeshes, m_nMaterials);

	pClone->m_xmf4x4Transform = m_xmf4x4Transform;
	pClone->m_xmf4x4World = m_xmf4x4World;

	for (int i = 0; i < m_nMeshes; ++i)
	{
		if (m_ppMeshes[i])
		{
			pClone->m_ppMeshes[i] = m_ppMeshes[i];
			pClone->m_ppMeshes[i]->AddRef();
		}
	}
	...
	for (int i = 0; i < m_nMaterials; ++i)
	{
		if (m_ppMaterials[i])
		{
			pClone->m_ppMaterials[i] = m_ppMaterials[i];
			pClone->m_ppMaterials[i]->AddRef();

			if (m_ppMaterials[i]->m_pTexture)
			{
				pClone->m_ppMaterials[i]->m_pTexture = m_ppMaterials[i]->m_pTexture;
				pClone->m_ppMaterials[i]->m_pTexture->AddRef();
			}
		}
	}

	strcpy_s(pClone->m_pstrFrameName, sizeof(m_pstrFrameName), m_pstrFrameName);

	if (m_pChild)
		pClone->SetChild(m_pChild->Clone());

	if (m_pSibling)
		pClone->m_pSibling = m_pSibling->Clone();
...
CBillboardObject::CBillboardObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(1, 1)
{
	...
	XMFLOAT2 size = XMFLOAT2(0.f, 0.f);
	...
	CScene::CreateShaderResourceViews(pd3dDevice, pBillboardTexture, 0, 12);

	CMaterial* pBillboardMaterial = new CMaterial();
	pBillboardMaterial->SetTexture(pBillboardTexture);

	SetMaterial(0, pBillboardMaterial);
}
...
void CBillboardObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, UINT nInstances)
{
	CGameObject::Render(pd3dCommandList, pCamera, nInstances);
}
...
CBoundingBoxObject::CBoundingBoxObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(1, 1)
{
	...
	CMaterial* pBoundingBox = new CMaterial();

	SetMaterial(0, pBoundingBox);
}
...
void CBoundingBoxObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, UINT nInstances)
{
	CGameObject::Render(pd3dCommandList, pCamera, nInstances);
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
CRectObject::CRectObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(1, 1)
{
	XMFLOAT3 location = XMFLOAT3(0.f, 0.f, 0.f);
	XMFLOAT2 size = XMFLOAT2(0.f, 0.f);
...
void CRectObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, UINT nInstances)
{
	CGameObject::Render(pd3dCommandList, pCamera, nInstances);
}