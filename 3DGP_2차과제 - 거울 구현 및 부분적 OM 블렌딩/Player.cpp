...
void CPlayer::Update(float fTimeElapsed)
{
	...
	{
		float forwardOffset = 10.0f;   // 원하는 만큼 조절
		XMFLOAT3 offset = Vector3::ScalarProduct(m_xmf3Look, forwardOffset, false);
		XMFLOAT3 newCamPos = Vector3::Add(m_pCamera->GetPosition(), offset);

		//m_pCamera->SetPosition(newCamPos);
		m_pCamera->Update(newCamPos, fTimeElapsed);
	}
...
CCamera *CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	switch (nNewCameraMode)
	{
		case FIRST_PERSON_CAMERA:
			...
			m_pCamera->SetViewport(0, 0, ::g_nScreenWidth, ::g_nScreenHeight, 0.0f, 1.0f);
			m_pCamera->SetScissorRect(0, 0, ::g_nScreenWidth, ::g_nScreenHeight);
			break;
		case SPACESHIP_CAMERA:
			SetFriction(100.5f);
			SetGravity(XMFLOAT3(0.0f, 0.0f, 0.0f));
